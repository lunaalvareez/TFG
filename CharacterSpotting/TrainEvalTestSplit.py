...
all_image_files = [f for f in os.listdir(sentencesCompressedSubset_path) if f.endswith('.jpg')]
all_pairs = [(img, img.replace('.jpg', '.txt')) for img in all_image_files if os.path.exists(os.path.join(sentencesCompressedSubset_path, img.replace('.jpg', '.txt')))]
...
testset_pairs = [(img, img.replace('.jpg', '.txt')) for img in all_image_files if matches_identifier(img, testset_identifiers) and os.path.exists(os.path.join(sentencesCompressedSubset_path, img.replace('.jpg', '.txt')))]#filter pairs based on identifiers in testSet.txt for potential inclusion in testSet
