...
selected_pairs_for_trainEvalSet = random.sample(trainset_pairs, min(2000, len(trainset_pairs))) #randomly select up to 1500 pairs from those matching the identifiers
...
testset_pairs = [(img, img.replace('.jpg', '.txt')) for img in all_image_files if matches_identifier(img, testset_identifiers) and os.path.exists(os.path.join(wordsCompressedSubset_path, img.replace('.jpg', '.txt')))]#filter pairs based on identifiers in trainset.txt for potential inclusion in trainEvalSet

selected_pairs_for_testSet = random.sample(trainset_pairs, min(700, len(testset_pairs))) #randomly select up to 1500 pairs from those matching the identifiers
